...
/*Initialize the network load library, create sockets, bind IP addresses, apply for broadcast rights,
 *and create a receiver thread*/
bool TcpClientNet::InitNet()
{
	...
	/*htons converts integers to network byte order and integer variables
	 *from host byte order to network byte order*/
	addrServer.sin_port = htons(_DEF_TCP_PORT);
	//To bind the server IP address, bind any NIC to the server
...
{
	//Verify parameters to determine the validity of the passed parameters
	if (!buf || nLen <= 0)
	{
		cout << "TcpClient::SendData parameter error" << endl;
		return false;
	}
	//To prevent sticky packets, send the packet size first, where m_sock is equal to ISendip
...
void TcpClientNet::RecvData()
{
	...
	int packSize = 0;
    //Offset, which calculates the cumulative size of accepted packets
	int offset = 0;
	while (!m_isStop)
	{
		...
		/*packSize records how much data has not been received in a package,
