...
//Call the core handler class function, in the mediator class, through the static core handler class
CKernel* CKernel::pKernel = NULL;

CKernel::CKernel(void)
{
    m_pServer= new TcpServerMediator;
    //Static member Pointers are initialized outside the class and assigned the value this in the constructor
	pKernel = this;
	//Initialize the array, call
	setProtocalMp();
}
...
{
	cout << "CKernel::setProtocalMp" << endl;
	memset(m_netProtocalMap, 0, _DEF_PROTOCAL_COUNT);
	//Bind the protocol header to the handler
	NetPackFunmap(_DEF_PACK_TCP_REGISTER_RQ)  = &CKernel::dealRegisterRq;
	NetPackFunmap(_DEF_PACK_TCP_LOGIN_RQ)     = &CKernel::dealLoginRq;
	...
	NetPackFunmap(_DEF_PACK_TCP_OFFLINE_RQ)   = &CKernel::dealOfflioneRq;
}
...
bool CKernel::startServer()
{
	...
	if (!sql.ConnectMySql("127.0.0.1", "root", "Ljy123456", "communication_schema")){
